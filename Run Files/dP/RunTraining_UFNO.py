...
Description:
    ...
    This script loads configuration settings from Config_UFNO.py,
    initializes the model, dataset, optimizer, and scheduler,
    and manages the full training and evaluation pipeline.

Usage:
    python RunTraining.py [--model_name NAME] [--problem_type TYPE]
                          [--lambda_r FLOAT] [--lambda_z FLOAT]
...
from Config.dP.Config_UFNO import (
    Model_Config,
    Dataset_Config,
    Optimizer_Config,
    Scheduler_Config,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f'Weight for radial derivative loss term (default: {Model_Config["Lambda_r"]})'
    )
    parser.add_argument(
        "--lambda_z",
        type=float,
        default=Model_Config["Lambda_z"],
        help=f'Weight for vertical derivative loss term (default: {Model_Config["Lambda_z"]})'
    )
